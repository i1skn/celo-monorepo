...
class CheckBuilder {
  ...
  isNotAccount = (address: Address) =>
    this.addCheck(
      ...
      this.withLockedGold((lg) => negate(lg.isAccount(address)))
    )

  isSignerOrAccount = () =>
    this.addCheck(
      `${this.signer!} is Signer or Account`,
      this.withLockedGold(async (lg) => {
        const res = (await lg.isAccount(this.signer!)) || (await lg.isSigner(this.signer!))
        return res
      })
    )

  isAccount = (address: Address) =>
    this.addCheck(`${address} is Account`, this.withLockedGold((lg) => lg.isAccount(address)))
